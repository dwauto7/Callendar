      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {snapshotRows.map(({ label, value, icon: Icon, delta: d }) => (
            <Card key={label} className="rounded-2xl border border-white/5 bg-white/[0.02] glass-panel">
              <CardContent className="p-5">
                ...
                <p className="text-2xl font-bold text-white tracking-tighter tabular-nums" style={{ fontFamily: 'var(--font-syne)' }}>
                  {value}
                </p>
        ...
        <Card className="mt-4 rounded-2xl border border-white/5 bg-white/[0.02] glass-panel">
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm font-black text-white tabular-nums">{conversionRate}%</span>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {allReports.map((r, i) => {
                    ...
                    return (
                      <tr key={r.id} className={cn('hover:bg-white/[0.02] transition-colors', i === 0 && 'bg-[#40E0FF]/[0.02]')}>
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold text-white/60">{r.report_period}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-16 h-1.5 rounded-full bg-black/40 overflow-hidden">
                              <div className="h-full rounded-full bg-[#40E0FF]" style={{ width: `${eff}%` }} />
                            </div>
                            <span className="text-xs font-bold text-white/50 tabular-nums">{eff}%</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                        ...
                        <td className="px-6 py-4">
                          <span className="text-xs font-bold text-white/50 tabular-nums">{formatMins(r.total_minutes_used)}</span>
                        </td>
                      </tr>
                    )
                  })}
...
function ChartHeader({ icon: Icon, title, subtitle }: { icon: React.ElementType; title: string; subtitle: string }) {
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <h2 className="text-sm font-black text-white uppercase tracking-widest" style={{ fontFamily: 'var(--font-syne)' }}>{title}</h2>
        <p className="text-[10px] text-white/30 font-bold uppercase tracking-tighter mt-0.5">{subtitle}</p>
      </div>
    </div>
  )
}
...
function DeltaBadge({ delta }: { delta: number | null }) {
  ...
  return (
    <span className={cn('flex items-center gap-0.5 text-[10px] font-black', positive ? 'text-emerald-400' : 'text-red-400')}>
